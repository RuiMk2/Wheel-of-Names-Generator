...
@login_required
def index():
    tablesOwned = extract(db.execute(
        "SELECT * FROM tables WHERE OwnerID = ? ORDER BY TableID DESC", (session["user_id"],)).fetchall())
...
def changeusername():
    if request.method == "POST":
        ...
        query = extract(db.execute(
            "SELECT * FROM users WHERE username = ?", (new,)).fetchall())
        # check if username is valid
        if not new:
...
def DeleteTable():
    ...
    if request.method == "POST":
        ...
        if request.form["choice"] == "Yes":
            db.execute("DELETE FROM tables WHERE OwnerID=? AND TableID=?",
...
def CopyTable():
    ...
    if len(check) == 0:
        return render_template("not found.html")
    # Create new table
    db.execute("INSERT INTO tables(OwnerID, TableName) VALUES(?,?)",
               (session["user_id"], check[0]["TableName"]))
    db.commit()
    # Get the latest table the user created
    NewTable = extract(db.execute(
        "SELECT * FROM tables WHERE OwnerID=? ORDER BY TableID DESC", (session["user_id"],)).fetchall())
    # Get the data from old table
    datas = extract(db.execute("SELECT * FROM data WHERE TableID=? AND OwnerID=?",
                    (TableID, session["user_id"])).fetchall())
...
def view():
    ...
    if not len(check) == 1:
        return render_template("not found.html")
    # Render the data
    data = extract(db.execute(
        "SELECT * FROM data WHERE TableID=?", (TableID, )).fetchall())
...
def edit():
    ...
    if not len(check) == 1:
        return render_template("not found.html")
    # Obtain the data
    data = extract(db.execute("SELECT * FROM data WHERE TableID=? AND OwnerID = ?",
                   (TableID, session["user_id"])).fetchall())
    ...
    if request.method == "POST":
        ...
        if request.form["action"] == "AddPerson":
            ...
            db.execute("INSERT INTO data(TableID, OwnerID, Name, Entries) VALUES(?,?,?,?)",
                       (TableID, session["user_id"], name, entries))
            db.commit()
        ...
        # Delete Selected People
            ...
            for i in range(len(delete)):
                try:
                    ...
                    db.execute("DELETE FROM data WHERE NameID=? AND OwnerID=?",
                               (delete[i], session["user_id"]))
                    db.commit()
                except:
                    return redirect(request.referrer)
            return redirect(request.referrer)
        # Update the table based on the editted values
            ...
            for i in range(len(ids)):
                ...
                try:
                    ...
                    entries[i] = 0
                try:
                    db.execute("UPDATE data SET Name=?, Entries=? WHERE NameID=? AND OwnerID=?", (
                        names[i], entries[i], ids[i], session["user_id"]))
                    db.commit()
                except:
                    return redirect(request.referrer)
            return redirect(request.referrer)
        # Rename the table
            ...
            if not TableName:
                return redirect(request.referrer)
            db.execute("UPDATE tables SET TableName=? WHERE TableID=? AND OwnerID=?",
                       (TableName, TableID, session["user_id"]))
            db.commit()
            return redirect(request.referrer)
        # Change publicity of the table
            ...
            if int(check[0]["IsPublic"]) == 0:
                db.execute("UPDATE tables SET IsPublic = 1 WHERE TableID=? AND OwnerID=?",
                           (TableID, session["user_id"]))
                db.commit()
            elif int(check[0]["IsPublic"]) == 1:
                db.execute("UPDATE tables SET IsPublic = 0 WHERE TableID=? AND OwnerID=?",
                           (TableID, session["user_id"]))
                db.commit()
